...
async function apiGet(action = "getAll") {
  ...
  } catch (err) { console.error("GET err:", err); return { success: false, error: err.message }; }
}
async function apiPost(body) {
  try {
    const res = await fetch(API_URL, { method: "POST", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify(body) });
    return await res.json();
  } catch (err) { console.error("POST err:", err); return { success: false, error: err.message }; }
}
...
function Spinner({ text = "Cargando..." }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 20px", gap: 12 }}>
      <div style={{ width: 36, height: 36, border: "4px solid #dde6cc", borderTopColor: "#4a7c28", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      <span style={{ fontSize: 14, color: "#6a7d55", fontWeight: 600 }}>{text}</span>
...
function Toast({ message }) {
  ...
  return <div style={{ textAlign: "center", padding: "10px 14px", background: "#2d5a10", borderRadius: 10, marginTop: 10, fontSize: 14, fontWeight: 600, color: "#fff", animation: "fadeIn 0.3s" }}>{message}</div>;
}
...
export default function App() {
  ...
  const saveCatalogs = async (cs, ci) => { await apiPost({ action: "updateConfig", data: { catServ: JSON.stringify(cs), catIns: JSON.stringify(ci) } }); };
  const updateCatServ = async (c) => { setCatServ(c); await saveCatalogs(c, catIns); };
  const updateCatIns = async (c) => { setCatIns(c); await saveCatalogs(catServ, c); };
  ...
  const addIngreso = async (data) => {
    const row = { ...data, id: uid(), liquidado: "false", liquidacionId: "" };
    setIngresos(p => [...p, { ...row, precioCobrado: num(row.precioCobrado), precioBase: num(row.precioBase), descuento: num(row.descuento), ingresoNeto: num(row.ingresoNeto), liquidado: false }]);
    await apiPost({ action: "addRow", sheet: "Ingresos", data: row });
  };

  const editIngreso = async (id, updates) => {
    const recalc = { ...updates };
    recalc.precioCobrado = num(recalc.precioCobrado);
    recalc.descuento = num(recalc.descuento);
    recalc.ingresoNeto = recalc.precioCobrado - recalc.descuento;
    setIngresos(p => p.map(i => i.id === id ? { ...i, ...recalc } : i));
    await apiPost({ action: "updateByIds", sheet: "Ingresos", ids: [id], updates: { ...recalc, precioCobrado: recalc.precioCobrado, descuento: recalc.descuento, ingresoNeto: recalc.ingresoNeto } });
  };

  const delIngreso = async (id) => {
    setIngresos(p => p.filter(i => i.id !== id));
    await apiPost({ action: "deleteById", sheet: "Ingresos", id });
  };

  const addInsumo = async (data) => {
    const row = { ...data, id: uid(), liquidado: "false", liquidacionId: "" };
    setInsumos(p => [...p, { ...row, cantidad: num(row.cantidad), costoUnitario: num(row.costoUnitario), costoTotal: num(row.costoTotal), liquidado: false }]);
    await apiPost({ action: "addRow", sheet: "Insumos", data: row });
  };

  const editInsumo = async (id, updates) => {
    const recalc = { ...updates };
    recalc.cantidad = num(recalc.cantidad);
    recalc.costoUnitario = num(recalc.costoUnitario);
    recalc.costoTotal = recalc.cantidad * recalc.costoUnitario;
    setInsumos(p => p.map(i => i.id === id ? { ...i, ...recalc } : i));
    await apiPost({ action: "updateByIds", sheet: "Insumos", ids: [id], updates: { ...recalc } });
  };

  const delInsumo = async (id) => {
    setInsumos(p => p.filter(i => i.id !== id));
    await apiPost({ action: "deleteById", sheet: "Insumos", id });
  };

  const doLiquidar = async (selIng, selIns) => {
    const li = ingresos.filter(i => selIng.includes(i.id));
    const ls = insumos.filter(i => selIns.includes(i.id));
    const tI = li.reduce((s, i) => s + (i.precioCobrado || 0), 0);
    const tD = li.reduce((s, i) => s + (i.descuento || 0), 0);
    const tS = ls.reduce((s, i) => s + (i.costoTotal || 0), 0);
    const nv = tI - tD; const ut = nv - tS;
    ...
    setLiquidaciones(p => [...p, liq]);
    setIngresos(p => p.map(i => selIng.includes(i.id) ? { ...i, liquidado: true, liquidacionId: liq.id } : i));
    setInsumos(p => p.map(i => selIns.includes(i.id) ? { ...i, liquidado: true, liquidacionId: liq.id } : i));
    await apiPost({ action: "addRow", sheet: "Liquidaciones", data: liq });
    if (selIng.length > 0) await apiPost({ action: "updateByIds", sheet: "Ingresos", ids: selIng, updates: { liquidado: "TRUE", liquidacionId: liq.id } });
    if (selIns.length > 0) await apiPost({ action: "updateByIds", sheet: "Insumos", ids: selIns, updates: { liquidado: "TRUE", liquidacionId: liq.id } });
  };

  const ingPend = ingresos.filter(i => !i.liquidado);
  const insPend = insumos.filter(i => !i.liquidado);

  if (loading) return (
    <div style={S.app}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,600;9..40,700;9..40,800&family=JetBrains+Mono:wght@500;700&display=swap');`}</style>
      <header style={S.header}><div style={S.headerTop}><div><h1 style={S.logo}>LAVADERO</h1><p style={S.logoSub}>Control Semanal</p></div></div></header>
      <Spinner text="Conectando con Google Sheets..." />
    </div>
  );
  ...
  return (
    <div style={S.app}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,600;9..40,700;9..40,800&family=JetBrains+Mono:wght@500;700&display=swap');
        * { box-sizing: border-box; margin: 0; padding: 0; }
        input, select, textarea, button { font-family: 'DM Sans', sans-serif; }
        input:focus, select:focus, textarea:focus { border-color: #4a7c28 !important; outline: none; box-shadow: 0 0 0 3px rgba(74,124,40,0.12); }
      ...
      <nav style={S.nav}>{TABS.map(t => (
        <button key={t.id} onClick={() => setTab(t.id)} style={{ ...S.navBtn, ...(tab === t.id ? S.navAct : {}) }}>
          <I d={t.icon} s={18} c={tab === t.id ? "#2d5a10" : "#9aab88"} />
          <span style={{ fontSize: 10, marginTop: 2, fontWeight: tab === t.id ? 700 : 500, color: tab === t.id ? "#2d5a10" : "#9aab88" }}>{t.label}</span>
        </button>
      ))}</nav>
      <main style={S.main}>
        ...
        {tab === "resumen" && <ResumenTab liquidaciones={liquidaciones} ingresos={ingresos} catServ={catServ} setCatServ={updateCatServ} catIns={catIns} setCatIns={updateCatIns} config={config} setConfig={updateConfig} subTab={subTab} setSubTab={setSubTab} />}
      </main>
    </div>
  );
}
...
function IngresosTab({ ingresos, addIngreso, editIngreso, delIngreso, catServ, config }) {
  ...
  const [ef, setEf] = useState(null); // edit form
  ...
  const save = async () => {
    if (!f.fecha || !f.placa || !f.servicioNombre || !precio) return;
    setSaving(true);
    const sel = catServ.find(s => s.nombre === f.servicioNombre);
    await addIngreso({ fecha: f.fecha, placa: f.placa, cliente: f.cliente, servicioNombre: f.servicioNombre, precioBase: sel?.precioBase || 0, precioCobrado: precio, descuento, ingresoNeto: neto, medioPago: f.medioPago, facturado: f.facturado, numFactura: f.numFactura, observaciones: f.observaciones });
    setF(empty); setSaving(false);
    setToast("Ingreso guardado"); setTimeout(() => setToast(""), 2500);
  };

  const startEdit = (r) => {
    setEditId(r.id);
    setEf({ fecha: r.fecha || "", placa: r.placa || "", cliente: r.cliente || "", servicioNombre: r.servicioNombre || "", precioCobrado: String(r.precioCobrado || ""), descuento: String(r.descuento || ""), medioPago: r.medioPago || "efectivo", facturado: r.facturado || "no", numFactura: r.numFactura || "", observaciones: r.observaciones || "" });
  };

  const saveEdit = async () => {
    if (!ef) return;
    setSaving(true);
    const sel = catServ.find(s => s.nombre === ef.servicioNombre);
    await editIngreso(editId, { fecha: ef.fecha, placa: ef.placa, cliente: ef.cliente, servicioNombre: ef.servicioNombre, precioBase: sel?.precioBase || 0, precioCobrado: ef.precioCobrado, descuento: ef.descuento, ingresoNeto: num(ef.precioCobrado) - num(ef.descuento), medioPago: ef.medioPago, facturado: ef.facturado, numFactura: ef.numFactura, observaciones: ef.observaciones });
    setEditId(null); setEf(null); setSaving(false);
    setToast("Ingreso actualizado"); setTimeout(() => setToast(""), 2500);
  };
    ...
    <div style={S.sec}>
      ...
      {pendientes.length > 0 && (
        <div style={{ marginTop: 24 }}>
          <button onClick={() => setShowList(!showList)} style={S.expandBtn}>{showList ? "v" : ">"} Pendientes ({pendientes.length})</button>
          {showList && <div style={S.miniList}>{pendientes.map(r => (
            <div key={r.id} style={{ ...S.miniCard, borderColor: editId === r.id ? "#4a7c28" : undefined }}>
              {editId === r.id && ef ? (
                /* ── EDIT MODE ── */
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#4a7c28", marginBottom: 2 }}>Editando ingreso</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <input type="date" value={ef.fecha} onChange={e => setEf({ ...ef, fecha: e.target.value })} style={{ ...S.inp, flex: 1, minWidth: 130, fontSize: 13, padding: "7px 8px" }} />
                    <input value={ef.placa} onChange={e => setEf({ ...ef, placa: e.target.value.toUpperCase() })} placeholder="Placa" maxLength={7} style={{ ...S.inp, width: 80, fontSize: 13, padding: "7px 8px" }} />
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <div style={{ flex: 1 }}><label style={S.miniLabel}>Precio ($)</label><input type="number" value={ef.precioCobrado} onChange={e => setEf({ ...ef, precioCobrado: e.target.value })} style={{ ...S.inp, fontSize: 13, padding: "7px 8px" }} /></div>
                    <div style={{ flex: 1 }}><label style={S.miniLabel}>Descuento ($)</label><input type="number" value={ef.descuento} onChange={e => setEf({ ...ef, descuento: e.target.value })} style={{ ...S.inp, fontSize: 13, padding: "7px 8px" }} /></div>
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <select value={ef.medioPago} onChange={e => setEf({ ...ef, medioPago: e.target.value })} style={{ ...S.inp, flex: 1, fontSize: 13, padding: "7px 8px" }}><option value="efectivo">Efectivo</option><option value="transferencia">Transferencia</option></select>
                    <select value={ef.facturado} onChange={e => setEf({ ...ef, facturado: e.target.value, numFactura: e.target.value === "no" ? "" : ef.numFactura })} style={{ ...S.inp, flex: 1, fontSize: 13, padding: "7px 8px" }}><option value="no">No facturado</option><option value="si">Facturado</option></select>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                    <button onClick={() => { setEditId(null); setEf(null); }} style={S.smBtn}>Cancelar</button>
                    <button onClick={saveEdit} disabled={saving} style={{ ...S.smBtn, background: "#2d5a10", color: "#fff", borderColor: "#2d5a10" }}>{saving ? "..." : "Guardar"}</button>
                  </div>
                </div>
              ) : (
                /* ── VIEW MODE ── */
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div><span style={S.miniPlaca}>{r.placa}</span><span style={{ fontSize: 12, color: "#777", marginLeft: 8 }}>{fDate(r.fecha)}</span></div>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: "#2d5a10", fontSize: 14 }}>{fCOP(r.precioCobrado)}</span>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 6 }}>
                    <button onClick={() => startEdit(r)} style={{ ...S.smBtn, color: "#4a7c28" }}>Editar</button>
                    <button onClick={() => delIngreso(r.id)} style={{ ...S.smBtn, color: "#c0392b" }}>Eliminar</button>
                  </div>
                </>
              )}
            </div>
...
function InsumosTab({ insumos, addInsumo, editInsumo, delInsumo, catIns }) {
  ...
  const handleInsumoChange = (nombre) => {
    const found = catIns.find(c => c.nombre === nombre);
    setF({ ...f, insumoNombre: nombre, costoUnitario: found ? String(found.costoUnitario) : f.costoUnitario, proveedor: found?.proveedor || f.proveedor });
  };

  const cant = num(f.cantidad); const costoU = num(f.costoUnitario); const costoTotal = cant * costoU;
  const catItem = catIns.find(c => c.nombre === f.insumoNombre);
  const costDiffers = catItem && costoU > 0 && costoU !== catItem.costoUnitario;

  const save = async () => {
    if (!f.fecha || !f.insumoNombre || !cant || !costoU) return;
    setSaving(true);
    const found = catIns.find(c => c.nombre === f.insumoNombre);
    await addInsumo({ fecha: f.fecha, insumoNombre: f.insumoNombre, cantidad: cant, costoUnitario: costoU, costoTotal, unidad: found?.unidad || "und", proveedor: f.proveedor, observaciones: f.observaciones });
    setF(empty); setSaving(false);
    setToast("Insumo guardado"); setTimeout(() => setToast(""), 2500);
  };

  const startEdit = (r) => {
    setEditId(r.id);
    setEf({ fecha: r.fecha || "", insumoNombre: r.insumoNombre || "", cantidad: String(r.cantidad || ""), costoUnitario: String(r.costoUnitario || ""), proveedor: r.proveedor || "", observaciones: r.observaciones || "" });
  };

  const saveEdit = async () => {
    if (!ef) return;
    setSaving(true);
    const found = catIns.find(c => c.nombre === ef.insumoNombre);
    await editInsumo(editId, { fecha: ef.fecha, insumoNombre: ef.insumoNombre, cantidad: ef.cantidad, costoUnitario: ef.costoUnitario, costoTotal: num(ef.cantidad) * num(ef.costoUnitario), unidad: found?.unidad || "und", proveedor: ef.proveedor, observaciones: ef.observaciones });
    setEditId(null); setEf(null); setSaving(false);
    setToast("Insumo actualizado"); setTimeout(() => setToast(""), 2500);
  };
  ...
  return (
    <div style={S.sec}>
      ...
      <div style={S.formGrid}>
        <FI label="Fecha" span={2}><input type="date" value={f.fecha} onChange={e => setF({ ...f, fecha: e.target.value })} style={S.inp} /></FI>
        <FI label="Insumo" span={2}><div style={S.selW}><select value={f.insumoNombre} onChange={e => handleInsumoChange(e.target.value)} style={S.sel}><option value="">-- Seleccionar --</option>{catIns.map((s, i) => <option key={i} value={s.nombre}>{s.nombre} ({s.unidad} - {fCOP(s.costoUnitario)})</option>)}</select><span style={S.selArr}>&#9662;</span></div></FI>
        ...
        <FI label="Costo unitario ($)">
          <input type="number" placeholder="0" value={f.costoUnitario} onChange={e => setF({ ...f, costoUnitario: e.target.value })} style={{ ...S.inp, borderColor: costDiffers ? "#e67e22" : undefined }} />
          {costDiffers && <div style={{ fontSize: 11, color: "#e67e22", marginTop: 3, fontWeight: 600 }}>Diferente al catalogo ({fCOP(catItem.costoUnitario)})</div>}
        </FI>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {showList && <div style={S.miniList}>{pendientes.map(r => (
            <div key={r.id} style={{ ...S.miniCard, borderColor: editId === r.id ? "#4a7c28" : undefined }}>
              {editId === r.id && ef ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#4a7c28", marginBottom: 2 }}>Editando insumo</div>
                  <input type="date" value={ef.fecha} onChange={e => setEf({ ...ef, fecha: e.target.value })} style={{ ...S.inp, fontSize: 13, padding: "7px 8px" }} />
                  <div style={S.selW}><select value={ef.insumoNombre} onChange={e => { const found = catIns.find(c => c.nombre === e.target.value); setEf({ ...ef, insumoNombre: e.target.value, costoUnitario: found ? String(found.costoUnitario) : ef.costoUnitario }); }} style={{ ...S.sel, fontSize: 13, padding: "7px 8px" }}><option value="">-- Insumo --</option>{catIns.map((s, i) => <option key={i} value={s.nombre}>{s.nombre}</option>)}</select><span style={S.selArr}>&#9662;</span></div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <div style={{ flex: 1 }}><label style={S.miniLabel}>Cantidad</label><input type="number" value={ef.cantidad} onChange={e => setEf({ ...ef, cantidad: e.target.value })} style={{ ...S.inp, fontSize: 13, padding: "7px 8px" }} /></div>
                    <div style={{ flex: 1 }}><label style={S.miniLabel}>Costo unit. ($)</label><input type="number" value={ef.costoUnitario} onChange={e => setEf({ ...ef, costoUnitario: e.target.value })} style={{ ...S.inp, fontSize: 13, padding: "7px 8px" }} /></div>
                  </div>
                  {num(ef.cantidad) * num(ef.costoUnitario) > 0 && <div style={{ ...S.netoBadge, background: "#fef5e7", color: "#7d5a08", fontSize: 13 }}>Total: <strong style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fCOP(num(ef.cantidad) * num(ef.costoUnitario))}</strong></div>}
                ...
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div><span style={{ fontWeight: 700 }}>{r.insumoNombre}</span><span style={{ fontSize: 12, color: "#777", marginLeft: 8 }}>{fDate(r.fecha)}</span></div>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: "#c0392b", fontSize: 14 }}>{fCOP(r.costoTotal)}</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>{r.cantidad} x {fCOP(r.costoUnitario)}{r.proveedor ? " - " + r.proveedor : ""}</div>
                  {r.observaciones && <div style={{ fontSize: 11, color: "#999", marginTop: 2, fontStyle: "italic" }}>{r.observaciones}</div>}
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 6 }}>
                    <button onClick={() => startEdit(r)} style={{ ...S.smBtn, color: "#4a7c28" }}>Editar</button>
                    <button onClick={() => delInsumo(r.id)} style={{ ...S.smBtn, color: "#c0392b" }}>Eliminar</button>
                  </div>
                </>
              )}
            </div>
...
function LiquidarTab({ ingPend, insPend, onLiquidar }) {
  ...
  const sI = ingPend.filter(i => selIng.includes(i.id));
  const sS = insPend.filter(i => selIns.includes(i.id));
  const tIng = sI.reduce((s, i) => s + (i.precioCobrado || 0), 0);
  const tDesc = sI.reduce((s, i) => s + (i.descuento || 0), 0);
  const tIns = sS.reduce((s, i) => s + (i.costoTotal || 0), 0);
  const neto = tIng - tDesc; const utilidad = neto - tIns; const mitad = utilidad / 2;

  const confirmar = async () => {
    if (selIng.length === 0 || saving) return;
    setSaving(true); await onLiquidar(selIng, selIns); setSaving(false);
    setDone(true); setTimeout(() => setDone(false), 3000);
  };

  if (ingPend.length === 0 && insPend.length === 0) return (<div style={S.sec}><SH title="Liquidacion" /><div style={S.empty}>No hay pendientes. Semana limpia!</div></div>);

  return (
    <div style={S.sec}>
      <SH title="Liquidacion" subtitle="Selecciona que entra" />
      {done && <Toast message="Liquidacion guardada en Google Sheets" />}
      <div style={S.liqSection}>
        <div style={S.liqHead}><h3 style={S.liqTitle}>Ingresos ({ingPend.length})</h3><button onClick={allIng} style={S.selAllBtn}>{selIng.length === ingPend.length ? "Deseleccionar" : "Seleccionar"} todos</button></div>
        <div style={S.checkList}>{ingPend.map(r => (
          <label key={r.id} style={{ ...S.checkItem, ...(selIng.includes(r.id) ? S.checkItemSel : {}) }}>
            <input type="checkbox" checked={selIng.includes(r.id)} onChange={() => togIng(r.id)} style={S.chk} />
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span><strong>{r.placa}</strong> <span style={{ color: "#777", fontSize: 12 }}>{r.servicioNombre}</span></span><span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: "#2d5a10" }}>{fCOP(r.precioCobrado)}</span></div>
              <div style={{ fontSize: 11, color: "#999" }}>{fDate(r.fecha)} - {r.medioPago}{r.descuento > 0 ? " - desc:" + fCOP(r.descuento) : ""}</div>
            </div>
          </label>
        ))}</div>
      </div>
      <div style={S.liqSection}>
        <div style={S.liqHead}><h3 style={S.liqTitle}>Insumos ({insPend.length})</h3>{insPend.length > 0 && <button onClick={allIns} style={S.selAllBtn}>{selIns.length === insPend.length ? "Deseleccionar" : "Seleccionar"} todos</button>}</div>
        {insPend.length === 0 ? <div style={{ fontSize: 13, color: "#999", padding: 10 }}>Sin insumos pendientes</div> : (
          <div style={S.checkList}>{insPend.map(r => (
            <label key={r.id} style={{ ...S.checkItem, ...(selIns.includes(r.id) ? { ...S.checkItemSel, borderColor: "#e8c8a0" } : {}) }}>
              <input type="checkbox" checked={selIns.includes(r.id)} onChange={() => togIns(r.id)} style={S.chk} />
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontWeight: 600 }}>{r.insumoNombre}</span><span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, color: "#c0392b" }}>{fCOP(r.costoTotal)}</span></div>
                <div style={{ fontSize: 11, color: "#999" }}>{fDate(r.fecha)} - {r.cantidad} x {fCOP(r.costoUnitario)}</div>
              </div>
            </label>
          ))}</div>
        )}
      ...
      {selIng.length > 0 && (
        <div style={S.liqCalc}>
          <h3 style={{ margin: "0 0 12px", fontSize: 16, fontWeight: 800, color: "#1a2e05" }}>Resumen</h3>
...
/* ═══════════════════════════════════════════════
   RESUMEN TAB
   ═══════════════════════════════════════════════ */
function ResumenTab({ liquidaciones, ingresos, catServ, setCatServ, catIns, setCatIns, config, setConfig, subTab, setSubTab }) {
  return (
    <div style={S.sec}>
      ...
      {subTab === "dashboard" && <DashSub liquidaciones={liquidaciones} />}
      {subTab === "historial" && <HistSub liquidaciones={liquidaciones} ingresos={ingresos} />}
      {subTab === "catalogos" && <CatSub catServ={catServ} setCatServ={setCatServ} catIns={catIns} setCatIns={setCatIns} config={config} setConfig={setConfig} />}
    </div>
  );
}

function DashSub({ liquidaciones }) {
  ...
  if (liquidaciones.length === 0) return <div style={S.empty}>Realiza tu primera liquidacion.</div>;
  return (<div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 8 }}>
      <SC label="Liquidaciones" value={st.count} /><SC label="Servicios" value={st.srv} /><SC label="Ingresos" value={fCOP(st.ing)} hi /><SC label="Insumos" value={fCOP(st.ins)} /><SC label="Utilidad" value={fCOP(st.util)} hi /><SC label="Propietario" value={fCOP(st.prop)} color="#2d5016" /><SC label="Trabajadores" value={fCOP(st.trab)} color="#5b7a3a" />
    </div>
  </div>);
}
...
function HistSub({ liquidaciones, ingresos }) {
  ...
  if (sorted.length === 0) return <div style={S.empty}>No hay liquidaciones.</div>;
  return (<div><h3 style={{ fontSize: 18, fontWeight: 800, color: "#1a2e05", marginBottom: 12 }}>Historial</h3>
    {sorted.map(l => { const li = l.ingresosIds ? ingresos.filter(i => l.ingresosIds.split(",").includes(i.id)) : []; return (
      <div key={l.id} style={{ ...S.dashCard, marginBottom: 10, cursor: "pointer" }} onClick={() => setOpen(open === l.id ? null : l.id)}>
        ...
        {open === l.id && (<div style={{ marginTop: 12, borderTop: "1px solid #e8eddf", paddingTop: 10 }}>
          {li.length > 0 && <>{li.map((r, i) => (<div key={i} style={{ fontSize: 12, padding: "3px 0", display: "flex", justifyContent: "space-between", borderBottom: "1px solid #f4f7ef" }}><span>{r.placa} - {r.servicioNombre}</span><span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fCOP(r.precioCobrado)}</span></div>))}</>}
          <div style={{ marginTop: 8 }}><LR label="Ingresos" value={fCOP(l.totalIngresos)} /><LR label="Descuentos" value={"-" + fCOP(l.totalDescuentos)} color="#c0392b" /><LR label="Insumos" value={"-" + fCOP(l.totalInsumos)} color="#c0392b" /><div style={S.liqDiv} /><LR label="Utilidad" value={fCOP(l.utilidadBruta)} bold /></div>
        </div>)}
      </div>); })}
  </div>);
}

function CatSub({ catServ, setCatServ, catIns, setCatIns, config, setConfig }) {
  ...
  return (<div>
    <div style={{ ...S.dashCard, marginBottom: 16, borderColor: "#c5d4a8" }}>
      <h4 style={{ ...S.dashCardT, marginBottom: 10 }}>Descuento Administrativo</h4>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <FI label="Monto ($)" compact><input type="number" value={config.descuento} onChange={e => setConfig({ ...config, descuento: num(e.target.value) })} style={{ ...S.inp, width: 120 }} /></FI>
        <FI label="Si precio >= ($)" compact><input type="number" value={config.umbral} onChange={e => setConfig({ ...config, umbral: num(e.target.value) })} style={{ ...S.inp, width: 120 }} /></FI>
      </div>
    </div>
    <h4 style={{ fontSize: 15, fontWeight: 800, color: "#1a2e05", marginBottom: 8 }}>Servicios</h4>
    <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
      <input placeholder="Nombre" value={sN} onChange={e => setSN(e.target.value)} style={{ ...S.inp, flex: 1, minWidth: 120 }} />
      <input type="number" placeholder="Precio base" value={sP} onChange={e => setSP(e.target.value)} style={{ ...S.inp, width: 110 }} />
      <button onClick={addS} disabled={!sN.trim() || !sP || sv2} style={{ ...S.primBtn, padding: "8px 14px", fontSize: 13, marginTop: 0, width: "auto", opacity: (!sN.trim() || !sP) ? 0.4 : 1 }}>+</button>
    </div>
    ...
    {catServ.length === 0 && <div style={{ fontSize: 13, color: "#999", padding: 10 }}>Sin servicios.</div>}
    <h4 style={{ fontSize: 15, fontWeight: 800, color: "#1a2e05", marginTop: 20, marginBottom: 8 }}>Insumos</h4>
    <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
      <input placeholder="Nombre" value={iN} onChange={e => setIN(e.target.value)} style={{ ...S.inp, flex: 1, minWidth: 100 }} />
      <select value={iU} onChange={e => setIU(e.target.value)} style={{ ...S.inp, width: 85, padding: "8px 4px" }}>{["bolsa", "galon", "litro", "unidad", "kg", "frasco", "caja"].map(u => <option key={u} value={u}>{u}</option>)}</select>
      <input type="number" placeholder="Costo" value={iC} onChange={e => setIC(e.target.value)} style={{ ...S.inp, width: 90 }} />
      <input placeholder="Prov." value={iP} onChange={e => setIP(e.target.value)} style={{ ...S.inp, width: 90 }} />
      <button onClick={addI} disabled={!iN.trim() || !iC || sv2} style={{ ...S.primBtn, padding: "8px 14px", fontSize: 13, marginTop: 0, width: "auto", opacity: (!iN.trim() || !iC) ? 0.4 : 1 }}>+</button>
    </div>
    ...
    {catIns.length === 0 && <div style={{ fontSize: 13, color: "#999", padding: 10 }}>Sin insumos.</div>}
  </div>);
}
...
/* ═══════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════ */
const S = {
  app: { fontFamily: "'DM Sans', sans-serif", background: "#f0f4e8", minHeight: "100vh", maxWidth: 540, margin: "0 auto", paddingBottom: 90, color: "#1a2e05" },
  header: { background: "linear-gradient(145deg, #0f1f03 0%, #1a3a06 40%, #2d5a10 100%)", padding: "20px 16px 12px" },
  ...
  navBtn: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "9px 2px 7px", border: "none", background: "transparent", cursor: "pointer", borderBottom: "3px solid transparent", transition: "all 0.15s" },
  ...
  sel: { width: "100%", padding: "9px 30px 9px 11px", border: "2px solid #d0dbb8", borderRadius: 10, fontSize: 14, background: "#fff", color: "#1a2e05", appearance: "none", cursor: "pointer", boxSizing: "border-box" },
  ...
  togBtn: { flex: 1, padding: "9px 12px", border: "2px solid #d0dbb8", borderRadius: 10, background: "#fff", fontSize: 13, fontWeight: 500, cursor: "pointer", color: "#777", transition: "all 0.15s", textAlign: "center" },
  ...
  xBtn: { background: "none", border: "none", cursor: "pointer", fontSize: 14, color: "#c0392b", padding: "2px 6px", borderRadius: 6, fontWeight: 700 },
